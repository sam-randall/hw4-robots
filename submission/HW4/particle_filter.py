...
class ParticleFilter(object):
    ...
    def transition_update(self, u, dt):
        ...
        new_u = u[np.newaxis, :] # new_u is going to have shape 1 x 2 (M x 2, where M = 1)

        us = np.repeat(new_u, M, axis = 0)
        g = self.transition_model(us, dt)
        
        for i in range(M):

             # g is now the new state for particle i

            # g : belief mean.
            # 1 x 3, 3 x 3 covariance matrix - what do we use for this? 
            # how do we incorporate the weights at this step?
            

            self.xs[i] = np.random.multivariate_normal(g[i], np.eye(3) * EPSILON_OMEGA)
...
class MonteCarloLocalization(ParticleFilter):
    ...
    def transition_model(self, us, dt):
        ...
        for i in range(M):
            particle = self.xs[i] # 1 x 3
            u = us[i] # 1 x 2.
            g_i = tb.compute_dynamics(particle, u, dt, compute_jacobians=False)
            g[i] = g_i
