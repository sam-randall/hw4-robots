...
class Ekf(object):
    ...
    def transition_update(self, u, dt):
        ...
        self.x = g
        R = self.R # TODO fill in R.

        self.Sigma = np.dot(np.dot(Gx, self.Sigma), np.transpose(Gx))
        self.Sigma += dt * np.dot((np.dot(Gu, R)), np.transpose(Gu))
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        S = np.dot(H, self.Sigma)
        S = np.dot(S, np.transpose(H)) + Q
        
        K = np.dot(self.Sigma, np.transpose(H))
        K = np.dot(K, np.linalg.inv(S))
        # what is xbar t
        dot = np.dot(K, z)
        self.x += dot
        
        self.Sigma -= np.dot(np.dot(K, S), np.transpose(K))
...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        z = np.array(v_list).reshape((K * 2,))

        
        
        for k in range(K):
            H[2 *k: 2*k + 2] = H_list[k] # 2 x 3
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(I):
            ...
            for j in range(J):
                ...
                H_j = Hs[j] # 2 x 3
                H_j_t = np.transpose(H_j) # 3 x 2
                
                
                
                S_ij = np.dot(H_j, self.Sigma)
                S_ij = np.dot(S_ij, H_j_t)
                ...
                d_ij = np.dot(np.transpose(v_ij), np.linalg.inv(S_ij))
                d_ij = np.dot(d_ij, v_ij)
                
                
                # if (d_ij.shape[0] != 1 or d_ij.shape[1] != 1):
                #     print("D_ij shape", d_ij.shape)
                #     assert(False)
                
                if d_ij < self.g ** 2 and d_ij < closest_distance:
                    closest_distance = d_ij
                    index_of_closest = j
            if closest_distance < self.g ** 2:
                v_list.append(V[i, index_of_closest, :]) # I think right here we want to use map_lines.
                Q_list.append(Q_raw[i])
                H_list.append(Hs[index_of_closest])
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        state = self.x[:3]
        features  = self.x[3:]
        g_dyn, Gx_dyn, Gu_dyn = tb.compute_dynamics(state, u, dt) # how do we incorporate features? 
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        z = np.zeros((K * 2))
        Q = np.zeros((2 * K, 2 * K))
        H = np.zeros((K * 2, H_list[0].shape[1])) # need to change 3.
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            #       alpha   r
            # h[0]: 1       0
            # h[1]  np.sqrt(x_cam ** 2 + y_cam ** 2) * sin(alpha - th_cam) 1

            # first three columns is what we got.
            Hx[:, :3] = temp_hx
            # after these three columns we have our first alpha r pair.
            Hx[:, 3:] = 0

            # (r - h[1]) / np.cos(alpha - th_cam)  =  np.sqrt(x_cam ** 2 + y_cam ** 2)
            sqrt_xc_yc = (r - h[1]) / np.cos(h[0])

            val = sqrt_xc_yc * np.sin(h[0])
